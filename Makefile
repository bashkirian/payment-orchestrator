...
loadtest-create: ## Run create payout load test
	docker run --rm --network host -v $(PWD)/loadtests:/loadtests $(K6_IMAGE) run /loadtests/scripts/create-payout.js

.PHONY: loadtest-full-flow
loadtest-full-flow: ## Run full payout flow load test
	docker run --rm --network host -v $(PWD)/loadtests:/loadtests $(K6_IMAGE) run /loadtests/scripts/full-flow.js

.PHONY: loadtest-rate-limit
loadtest-rate-limit: ## Run rate limiter stress test
	docker run --rm --network host -v $(PWD)/loadtests:/loadtests $(K6_IMAGE) run /loadtests/scripts/rate-limit-stress.js
